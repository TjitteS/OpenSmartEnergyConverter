...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs){
	if(RxFifo0ITs | FDCAN_IT_RX_FIFO0_NEW_MESSAGE){
		while(HAL_FDCAN_GetRxFifoFillLevel(&hfdcan2, FDCAN_RX_FIFO0) > 0){
			HAL_FDCAN_GetRxMessage(&hfdcan2, FDCAN_RX_FIFO0 ,&(RXQue[rx_wr].rxmsg), (RXQue[rx_wr].data));
			rx_wr++;
			rx_available++;
		...
		switch(packetid){
			...
			case CAN_PACKET_FILL_RX_BUFFER:
				if((rxmsg->data[0]+DLC) < MODCAN_RXBUFFER_SIZE){
					memcpy(modCANRxBuffer + rxmsg->data[0], rxmsg->data + 1, (DLC - 1)*sizeof(uint8_t));
				}
				break;
			...
			case CAN_PACKET_FILL_RX_BUFFER_LONG:
				...
				if(rxbuf_ind < RX_CAN_BUFFER_SIZE) {
					memcpy(modCANRxBuffer + rxbuf_ind, rxmsg->data + 2, (DLC - 2)*sizeof(uint8_t));
				}
				break;
...
#ifdef CAN_RX_DISABLE
			case CAN_PACKET_SetMode:
			case CAN_PACKET_SetOutputVoltage:
			case CAN_PACKET_SetInputVoltage:
			case CAN_PACKET_SetInputCurrent:
			case CAN_PACKET_SetOutputCurrent:
				break;
#else
			case CAN_PACKET_SetMode:
				if(DLC >= 1){
					modMpptSetMode(rxmsg->data[2]);
				}
				break;
			case CAN_PACKET_SetOutputVoltage:
				if(DLC >= 2){
					ind=0;
					modConverterSetOutputVoltage(1e3f * buffer_get_float16(rxmsg->data, 1e2, &ind));;
				}
				break;
			case CAN_PACKET_SetInputVoltage:
				if(DLC >= 2){
					ind=0;
					modConverterSetInputVoltage(1e3f * buffer_get_float16(rxmsg->data, 1e2, &ind));;
				}
				break;
			case CAN_PACKET_SetInputCurrent:
				if(DLC >= 2){
					ind=0;
					modConverterSetInputCurrent(1e3f * buffer_get_float16(rxmsg->data, 1e3, &ind));;
				}
				break;
			case CAN_PACKET_SetOutputCurrent:
				if(DLC >= 2){
					ind=0;
					modConverterSetOutputCurrent(1e3f * buffer_get_float16(rxmsg->data, 1e3, &ind));;
				}
				break;
#endif
			default:
...
void modCANtask(void){
	...
	if(tx_available){
		if(HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan2)){
			...
			HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &(TXQue[tx_rd].txmsg), TXQue[tx_rd].data);
			tx_rd++;
			tx_available--;
	...
	if(modDelayTick1ms(&lasttick_canmsg_stat, 1000)){
		...
		modCANTransmitStandardID(modCANGetCANID(canid,CAN_PACKET_status),data,(uint32_t)index);
	}
	...
	if(modDelayTick1ms(&lasttick_canmsg_power, 500)){
		...
		buffer_append_float16(data, meter.Vin, 2.5e2f, &index);
		buffer_append_float16(data, meter.Iin, 2.0e3f, &index);
		buffer_append_float16(data, meter.Vout, 2.5e2f, &index);
		buffer_append_float16(data, meter.Iout, 2.0e3f, &index);

		modCANTransmitStandardID(modCANGetCANID(canid,CAN_PACKET_Power),data,(uint32_t)index);
	}
}
...
void modCANTransmitStandardID(uint32_t id, uint8_t *data, uint8_t len){
	...
	tx_wr++;
	tx_available++;
	if(tx_wr >= MODCAN_TXBUFFER_SIZE)tx_wr=0;
}
